...
class RealSenseViewer:
    ...
    # loading the data stored in attached json file to the camera and class variables if its existing
    ...
    # here the functions are called in sequence to create the window
    # changing the order of the functions calls may result in errors because some of them are dependent of each other's
    # GUI elements that are called
    def create_gui(self):
    ...
    # function where all the top bar GUI elements are created, but also the scheduled process is handled here
    # also functions for saving snapshots and opening the point cloud viewer are defined here
    def create_top_bar(self):

        # function to save simultaneously snapshots from Dof and color camera
        def save_png():
        ...
        # sends snapshots from cameras to export.ply file where they are transformed into 3D image using build-in
        # pyrealsense functions. Can be later replaced with own solution. We were struggling with the function changing
        # color red for blue
        def save_ply():
        ...
        # function to start point-cloud viewer from open source opencv project, can be later replaced with own solution
        ...
        # this function is used for getting the scheduling info from second window to main, but also it starts scheduled process
        # now it needs to be accessible for whole class to start process from loaded user settings file
        # possibly the check_cron and update_cron functions can be refactored for cleaner code and easier access
        # only to the part needed for loading user settings
        def parent_callback(cron_interval, callback_end_date):
            ...
            def check_cron():
                ...
                if self.next_execution > current_time:
                    ...
                    self.time_to_next_picture_label.configure(
                        text=f"Time to Next Picture: {round(self.time_until_next_execution, 0)} seconds")
        ...
        # this part is starting the cron process if the croniter value is set from the config file
    ...
    # saving all the camera and scheduling parameters to json file in working directory
    ...
    # creation of canvas GUI elements and refresh function
    def create_canvases(self):
        ...
        # if we cannot catch any frames from the camera stream, means it's not connected. Stop the programme
        def update_canvas():
            try:
        ...
        # if camera is connected and canvases are initiated start a refresh canvas process every 10 milliseconds
        def update():
            update_canvas()
    ...
    # this part is creating the GUI part where scheduling
    def create_schedule_info(self):
        ...
        self.time_to_next_picture_label = tk.Label(schedule_info_frame,
                                                   text=f"Time to Next Picture:{self.time_until_next_execution}")
    ...
    def update_auto_btns(self):
        self.auto_btns[0].configure(
            text="Auto White Balance" if state.realsense.get_color_auto_white_balance() else "Manual White Balance")
        self.auto_btns[1].configure(
            text="Auto Exposure" if state.realsense.get_color_auto_exposure() else "Manual Exposure")
        self.auto_btns[2].configure(
            text="Auto Exposure" if state.realsense.get_depth_auto_exposure() else "Manual Exposure")
